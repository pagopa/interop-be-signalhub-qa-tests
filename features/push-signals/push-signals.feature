...
Feature: Deposito segnali
  ...
  @push_signals2
  Scenario Outline: Un utente abilitato all'e-service deposito segnali deposita un segnale vuoto. La richiesta va in errore. NB. verificare status code
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
  ...
  @push_signals4
  Scenario Outline: Un utente abilitato all'e-service deposito segnali deposita un segnale con un signalType errato. La richiesta va in errore. NB. verificare status code.
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
  ...
  @push_signals7
  Scenario Outline: Un utente la cui organizzazione ha una richiesta di fruizione per l’e-service deposito segnali, deposita un segnale per un e-service di cui non è erogatore. La richiesta va in errore. NB. Nonostante l'organizzazione abbia una richiesta di fruizione per deposito segnali, deposita un segnale per un e-service che apppartiene a un'altra organizzazione
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
  ...
  @push_signals8a
  Scenario Outline: Un utente la cui organizzazione ha una richiesta di fruizione per l’e-service deposito segnali, deposita un segnale per un e-service non esistente. La richiesta va in errore
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
    When l'utente deposita un segnale per un e-service che non esiste
    Then la richiesta va in errore con status code 403

  @push_signals8b
  Scenario Outline: Un utente la cui organizzazione ha una richiesta di fruizione per l’e-service deposito segnali, deposita un segnale per un e-service non pubblicato. La richiesta va in errore
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
    When l'utente deposita un segnale per un e-service che non è stato pubblicato
    Then la richiesta va in errore con status code 403

  @push_signals9
  Scenario Outline: Un utente abilitato all'e-service deposito segnali deposita due segnali con lo stesso signalId. La seconda richiesta va in errore: viene rifiutata perché signalId duplicato. NB. Viene introdotto un timeout per consentire al sistema la prima scrittura, dato che avviene con un collaborazione tra processi asincroni.
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
  ...
  @push_signals10
  Scenario Outline: Un utente la cui organizzazione non ha una richiesta di fruizione per deposito segnali ma per un altro e-service, deposita un segnale. La richiesta va in errore. NB. Non è possibile depositare un segnale senza una richiesta di fruizione attiva verso l'e-service di deposito segnali. Il processo di autenticazione dell'e-service deposito segnali considera il voucher non valido a causa dell'audience non corrispondente a quello dell'e-service deposito segnali. Il claim "aud" è impostato dell'authentication server in base alla purposeId presente nella client assertion
  ...
  # @push_signals11
  # Scenario Outline: L'utente deposita un segnale specifico per consumer
