...
Feature: Deposito segnali
  ...
  @push_signals7
  Scenario Outline: Un utente la cui organizzazione ha una richiesta di fruizione per l’e-service "deposito segnali", deposita un segnale per un e-service di cui non è erogatore.La richiesta non va a buon fine.
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
