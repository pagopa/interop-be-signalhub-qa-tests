...
Feature: Deposito segnali
  ...
  @push_signals8a
  Scenario Outline: Un utente la cui organizzazione ha una richiesta di fruizione per l’e-service "deposito segnali", deposita un segnale per un e-service non esistente. La richiesta non va a buon fine
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
    When l'utente deposita un segnale per un e-service che non esiste
    Then la richiesta non va a buon fine con status code 403

  @push_signals8b
  Scenario Outline: Un utente la cui organizzazione ha una richiesta di fruizione per l’e-service "deposito segnali", deposita un segnale per un e-service non pubblicato. La richiesta non va a buon fine
    Given Un utente, come produttore di segnali, ottiene un voucher valido per l'accesso all'e-service deposito segnali
  ...
  @push_signals10
  Scenario Outline: L'utente non ha una richiesta di fruizione per il service di deposito  segnali
