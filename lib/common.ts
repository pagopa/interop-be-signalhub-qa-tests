...
import { Agreement, Delegation, Eservice, Purpose } from "./data.interop";

export async function createOrUpdateAgreement(
  agreement: Agreement,
  organizationId: string,
): Promise<void> {
  const { descriptor, eservice, id, state } = agreement;
  const query = {
...
export async function createOrUpdateDelegation(delegation: Delegation) {
  const { delegateId, delegationId, delegatorId, eServiceId, kind, state } =
    delegation;
...
export async function createOrUpdateEservice(
  ...
  const { client_access_delegable, descriptor, enable_signal_hub, id, state } =
    eservice;
...
export async function createOrUpdatePurpose(
  ...
  const { delegationId, eservice, id, state, version } = purpose;
  const query = {
